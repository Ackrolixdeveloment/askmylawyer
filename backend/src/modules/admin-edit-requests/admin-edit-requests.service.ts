...
const shortCode = (prefix: string, id: string) => `${prefix}-${id.slice(0, 8).toUpperCase()}`;
...
export class AdminEditRequestsService {
  ...
  async history() {
    const rows = await this.prisma.lawyerEditRequest.findMany({
      ...
      select: {
        ...
        requestedAt: true,
        user: {
          select: { id: true, lawyerNumber: true, fullName: true, email: true, phone: true },
        },
      },
      orderBy: { requestedAt: 'desc' },
    });
    ...
    for (const row of rows) {
      const entry = byLawyer.get(row.user.id) ?? emptyHistoryRow(row.user, row.requestedAt);
  ...
  async approve(id: string, adminId: string) {
    ...
    const previousProof = row.user.documents.find((doc) => doc.type === 'bank_proof');
    const account = {
    ...
    if (previousKey && previousKey !== row.proofKey) await this.storage.delete(previousKey);
    void previousProof;

    await this.notifications.bankChangeApproved(this.recipient(row), payload.accountLast4);
  ...
  private toRequest(row: RequestRow) {
    ...
    const currentProof = row.user.documents.find((doc) => doc.type === 'bank_proof');

    return {
      id: row.id,
      requestId: shortCode('REQ', row.id),
      lawyerId: lawyerCode(row.user.lawyerNumber),
...
const notFound = () =>
  new AppException(HttpStatus.NOT_FOUND, 'REQUEST_NOT_FOUND', 'This request was not found.');

/** A history row before any request has been counted into it. */
function emptyHistoryRow(
  user: {
    id: string;
    lawyerNumber: number | null;
