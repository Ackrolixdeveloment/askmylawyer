...
export function AppSettingsForm({ initialRates }: AppSettingsFormProps) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-line pt-5">
          ...
          <Button onClick={() => setSaved(true)} disabled={!isValid}>
            Save changes
          </Button>
          {saved ? (
...
function RateRow({
  ...
  const split = priceBreakdown(rate);
...
function RateDetail({
  ...
  const split = priceBreakdown(rate, extensions);
        ...
        <div className="rounded-lg border border-line bg-surface p-4">
          ...
          {extensions > 0 ? (
            <>
              ...
              />
              <SplitRow
                label={`GST ${gstPercent}% on extensions`}
...
function ModeChoice({
  ...
  return (
    <label
      className={cn(
        "flex cursor-pointer items-center justify-center gap-1.5 rounded-lg border px-3 py-2.5 text-sm font-medium transition-colors",
        checked
